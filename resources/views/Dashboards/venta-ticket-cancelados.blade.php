<x-page-container title="Reporte de Tickets Cancelados">
    <x-card-gradient-header
        icon="ticket-perforated"
        title="Reporte de Tickets Cancelados"
        subtitle="Consulta de ventas detalladas por ticket cancelados"
    >
        <x-slot:buttons>
            <a
                href="/DashTicketsCancelados/exports?{{ http_build_query(request()->only(['idTienda', 'fecha_inicio', 'fecha_fin', 'fecha', 'id_ticket', 'id_encabezado', 'folio', 'status_venta', 'solicitud_fe', 'cancelado'])) }}"
        ...
        <!-- Filtros -->
        <x-form.form action="/DashTicketsCancelados">
            <x-form.group>
                <x-form.select
                    name="idTienda"
                    label="Tienda"
        ...
        <div>
                ...
                <div>
                    <h5 class="section-content-title">
                        <i
                            ...
                            style="color: #64748b;"
                        ></i>Reporte de Ventas Canceladas
                    </h5>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <div class="btn-group me-2">
                        <button
                            ...
                            style="background: #f1f5f9; color: #475569; border: none; border-radius: 6px 0 0 6px; padding: 6px 12px; font-size: 0.8rem;"
                        >
                            📋 Detallado
                        </button>
                        <button
                            ...
                            style="background: #1e293b; color: white; border: none; border-radius: 0 6px 6px 0; padding: 6px 12px; font-size: 0.8rem;"
                        >
                            🎫 Solo Tickets
                        </button>
                    </div>
            ...
            >
                <div class="table-responsive">
                    ...
                    {{-- ============ VISTA TICKETS CANCELADOS ============ --}}
                    <div id="vistaResumida">
                        <table class="table-hover table-custom table">
                            <thead>
                                <tr>
                                    ...
                                    <th><i class="bi bi-calendar3 me-1"></i>Fecha Venta</th>
                                    <th><i class="bi bi-calendar-x me-1"></i>Fecha Cancelación</th>
                                    <th class="text-center"><i class="bi bi-box me-1"></i>Artículos</th>
                                    <th class="text-start"><i class="bi bi-person-x me-1"></i>Cancelado por</th>
                                    <th class="text-start"><i class="bi bi-chat-dots me-1"></i>Motivo</th>
                                    <th class="text-end"><i class="bi bi-cash-stack me-1"></i>Importe</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php $totalImporteCancelado = 0; @endphp
                                @php $totalArticulos = 0; @endphp
                                @forelse ($groupedByTicket as $ticketId => $ticketData)
                                    @php
                                        $ticketInfo = $ticketData['ticket_info'];
                                        $totalImporteCancelado += $ticketData['totales']['importe'];
                                        $totalArticulos += count($ticketData['items']);
                                    @endphp
                                    <tr>
                                        {{-- Ticket ID --}}
                                        ...
                                        <td
                                            ...
                                            title="{{ $ticketInfo->NomTienda ?? 'N/A' }}"
                                        >
                                            {{ $ticketInfo->NomTienda ?? 'N/A' }}
                                        </td>
                                        ...
                                        {{-- Fecha Venta --}}
                                        <td style="font-size: 0.85rem;">
                                            {{ \Carbon\Carbon::parse($ticketInfo->FechaVenta)->locale('es')->format('d/m/Y H:i') }}
                                        </td>

                                        {{-- Fecha Cancelación --}}
                                        <td style="font-size: 0.85rem; color: #ef4444;">
                                            @if ($ticketInfo->FechaCancelacion)
                                                {{ \Carbon\Carbon::parse($ticketInfo->FechaCancelacion)->locale('es')->format('d/m/Y H:i') }}
                                            @else
                                                <span style="color: #94a3b8;">—</span>
                                            @endif
                                        </td>
                                        ...
                                        <td
                                            ...
                                            style="font-weight: 500;"
                                        >
                                            <span style="background: #f1f5f9; padding: 2px 8px; border-radius: 12px;">
                                        ...
                                        {{-- Usuario que canceló --}}
                                        <td style="font-size: 0.85rem;">
                                            @if ($ticketInfo->NombreUsuarioCancelacion)
                                                <span style="font-weight: 500;">
                                                    {{ $ticketInfo->NombreUsuarioCancelacion }}
                                                    {{ $ticketInfo->ApellidoUsuarioCancelacion }}
                                                </span>
                                            @else
                                                <span style="color: #94a3b8;">—</span>
                                            @endif
                                        ...
                                        <td style="max-width: 200px;">
                                            @if ($ticketInfo->MotivoCancel)
                                                <span
                                                    style="background: #fef2f2; color: #ef4444; padding: 3px 8px; border-radius: 12px; font-size: 0.8rem; display: inline-block; max-width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                                                    title="{{ $ticketInfo->MotivoCancel }}"
                                                >
                                                    {{ \Illuminate\Support\Str::limit($ticketInfo->MotivoCancel, 25) }}
                                                </span>
                                            @else
                                                <span style="color: #94a3b8;">—</span>
                                            @endif
                                        ...
                                        <td
                                            class="text-end"
                                            style="font-weight: 600; color: #ef4444;"
                                        >
                                ...
                                <tfoot>
                                    <tr style="background: #fef2f2; font-weight: 700;">
                                        <td
                                            ...
                                            style="color: #0f172a;"
                                        >
                                            Total Tickets Cancelados: {{ count($groupedByTicket) }}
                                        </td>
                                        ...
                                        >
                                            ${{ number_format($totalImporteCancelado, 2) }}
                                        </td>
                                    </tr>
                                </tfoot>
                            @endif
