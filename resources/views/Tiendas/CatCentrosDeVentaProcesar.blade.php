<x-page-container title="Procesar Cortes Ecommerce">
    <x-card-gradient-header
        icon="scissors"
        title="Procesar Cortes Ecommerce"
        subtitle="Gestione el procesamiento de cortes por centro de venta"
    >
        <x-slot:buttons>
        ...
        <div class="p-4">
                ...
                <div>
                    <h5 class="section-content-title">
                        <i
                            ...
                            style="color: #64748b;"
                        ></i>Centros de Venta Registrados
                    </h5>
                    <p class="section-content-subtitle">{{ count($centrosVenta) }} centros</p>
                </div>
            </div>
            ...
            >
                <table class="table-hover table-custom table">
                    ...
                    <tbody>
                        @forelse ($centrosVenta as $centroVenta)
                            <tr
                                class="row-clickable"
                            ...
                            >
                                <td style="font-weight: 600; color: #0f172a;">{{ $centroVenta->Almacen_Oracle }}</td>
                                ...
                                <td
                                    ...
                                    style="color: #64748b;"
                                >
                                    {{ $centroVenta->fechaprocesarcorte ? \Carbon\Carbon::parse($centroVenta->fechaprocesarcorte)->format('d/m/Y, h:i A') : '-' }}
                                </td>
                                <td>
                            ...
                            <tr>
                                <td colspan="6">
                                    <div class="py-5 text-center">
                                        ...
                                        <h6 class="text-muted">Sin centros de venta registrados</h6>
                                        <small class="text-muted">No se encontraron centros de venta disponibles</small>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        async function loadHistory(id, nombre, subinventario, reset = true) {
            ...
            try {
                ...
                if (reset) {
                    let html = `
                        <div class="mb-4">
                            <span style="font-size: 0.75rem; color: #94a3b8; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px;">Centro de Venta</span>
            ...
            input.addEventListener("change", async function(e) {
                ...
                try {
                    ...
                    if (data.ok) {
                        ...
                        if (tdUsuario && centroVenta.ceNombre) tdUsuario.textContent =
                            `${centroVenta.ceNombre} ${centroVenta.ceApellidos}`;
                        const tdFecha = document.querySelector(
                            `.col-fecha[data-id="${centroVenta.Almacen_Oracle}"]`);
                        if (tdFecha && centroVenta.fechaprocesarcorte) tdFecha.textContent =
                            formatearFecha(centroVenta.fechaprocesarcorte);
