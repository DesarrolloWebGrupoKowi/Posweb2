<x-page-container title="Procesar Cortes Tiendas">
    <x-card-gradient-header
        icon="scissors"
        title="Procesar Cortes Tiendas"
        subtitle="Gestione el procesamiento de cortes por tienda"
    >
        <x-slot:buttons>
        ...
        <x-form.form action="/CatTiendasProcesar" method="GET">
            <x-form.group>
                <x-form.text
                    name="filtroTienda"
                    label="Buscar tienda"
                ...
                <x-form.select
                    ...
                    :selected="request()->get('filtroStatus', '')"
                />
            </x-form.group>
            <div class="col-md-2 d-flex gap-2">
                <x-form.submit text="Filtrar" icon="funnel" class="flex-grow-1" />
                <x-form.clear />
            </div>
        </x-form.form>
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-lg-between align-items-lg-center mb-3 gap-3">
                <div>
                    <h5 class="section-content-title">
                        <i class="bi bi-table me-2" style="color: #64748b;"></i>Tiendas Registradas
                    </h5>
                    <p class="section-content-subtitle">{{ count($tiendas) }} tiendas</p>
                </div>
            </div>
                ...
                <table class="table-hover table-custom table">
                    <thead style="position: sticky; top: 0; z-index: 2; background: #f8fafc;">
                        <tr>
                            ...
                            <th><i class="bi bi-circle-fill me-1" style="font-size: 0.5rem;"></i>Estatus</th>
                            <th class="text-center"><i class="bi bi-scissors me-1"></i>Procesar Cortes</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($tiendas as $tienda)
                            <tr class="row-clickable" data-id="{{ $tienda->IdTienda }}" data-nombre="{{ $tienda->NomTienda }}" data-subinventario="{{ $tienda->IdTienda }}">
                                <td style="font-weight: 600; color: #0f172a;">{{ $tienda->IdTienda }}</td>
                                <td>{{ $tienda->NomTienda }}</td>
                                <td style="color: #64748b;">{{ $tienda->ccNomCiudad }}</td>
                                <td class="col-usuario" data-id="{{ $tienda->IdTienda }}">
                                    {{ $tienda->ceNombre ? $tienda->ceNombre . ' ' . $tienda->ceApellidos : '-' }}
                                </td>
                                <td class="col-fecha" data-id="{{ $tienda->IdTienda }}" style="color: #64748b;">
                                    {{ $tienda->fechaprocesarcorte ? \Carbon\Carbon::parse($tienda->fechaprocesarcorte)->format('d/m/Y, h:i A') : '-' }}
                                </td>
                                <td>
                                    @if ($tienda->Status == 0)
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-flex justify-content-center">
                                        <input class="form-check-input-modern toggle-estado" type="checkbox" role="switch"
                                ...
                                <td colspan="7">
                                    <div class="py-5 text-center">
                                        <div class="empty-state-icon mx-auto mb-3">
                                            <i class="bi bi-shop fs-3" style="color: #94a3b8;"></i>
                                        </div>
                                        <h6 class="text-muted">Sin tiendas registradas</h6>
                                        <small class="text-muted">No se encontraron tiendas con los filtros seleccionados</small>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <div class="side-panel" id="sidePanel">
        <div class="d-flex align-items-center justify-content-between mb-4 pb-3" style="border-bottom: 1px solid #e2e8f0;">
            <h5 class="fw-bold mb-0" style="color: #0f172a; font-size: 1rem;" id="sidePanelTitle">Historial de Cambios</h5>
            <button id="closeSidePanel" class="btn p-1" style="color: #94a3b8; font-size: 1.3rem; line-height: 1;">
...
<style>
    ...
    .row-clickable { cursor: pointer; transition: background-color 0.2s; }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelectorAll('.row-clickable').forEach(row => {
            row.addEventListener('click', function(e) {
                if (e.target.closest('.form-switch') || e.target.closest('.toggle-estado')) return;
        ...
        async function loadHistory(id, nombre, subinventario, reset = true) {
            ...
            try {
                ...
                const response = await fetch(`/CatTiendas/historial/${id}?page=${currentPage}&per_page=10`, {
                ...
                if (reset) {
                    let html = `
                        <div class="mb-4">
                            <span style="font-size: 0.75rem; color: #94a3b8; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px;">Tienda</span>
                    ...
                    if (data.historial && data.historial.length > 0) {
                        ...
                        data.historial.forEach(item => { html += renderHistoryItem(item); });
                        html += `</div>`;
                    ...
                    if (historyContainer && data.historial && data.historial.length > 0) {
                        data.historial.forEach(item => { historyContainer.innerHTML += renderHistoryItem(item); });
        ...
        function renderHistoryItem(item) {
            ...
            if (item.procesarcorteAnt !== null) valorAnterior = item.procesarcorteAnt == 0 ? 'Activado' : 'Desactivado';
            if (item.procesarcorte !== undefined) valorNuevo = item.procesarcorte == 0 ? 'Activado' : 'Desactivado';
            return `
            ...
            input.addEventListener("change", async function() {
                ...
                try {
                    const response = await fetch(`/CatTiendas/procesarcorte/${id}`, {
                        ...
                        body: JSON.stringify({ procesarcorte: nuevoEstado })
                    });
                    const data = await response.json();
                    if (data.ok) {
                        const tienda = data.tienda;
                        const tdUsuario = document.querySelector(`.col-usuario[data-id="${tienda.IdTienda}"]`);
                        if (tdUsuario && tienda.ceNombre) tdUsuario.textContent = `${tienda.ceNombre} ${tienda.ceApellidos}`;
                        const tdFecha = document.querySelector(`.col-fecha[data-id="${tienda.IdTienda}"]`);
                        if (tdFecha && tienda.fechaprocesarcorte) tdFecha.textContent = formatearFecha(tienda.fechaprocesarcorte);
                    } else {
        ...
        function formatearFecha(fecha) {
            ...
            return d.toLocaleString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }).toUpperCase();
        }
    });
</script>
