...
class CancelacionTicketsController extends Controller
{
    ...
    public function SolicitudCancelacionTicket(Request $request)
    {
        ...
        $ticket = DatEncabezado::with([
            ...
            ->when(strlen($idTicket) >= 8, fn($q) => $q->where('IdEncabezado', $idTicket))
            ->when(strlen($idTicket) < 8, fn($q) => $q->where('IdTicket', $idTicket))
            ->when(strlen($idTicket) < 8, fn($q) => $q->whereDate('FechaVenta', date('d-m-Y')))
