...
class ReportesController extends Controller
{
    ...
    public function ReportePorTipoDePrecio(Request $request)
    {
        ...
        return $concentrado = DB::table('DatEncabezado as a')
            ->leftJoin('DatDetalle as b', 'b.IdEncabezado', 'a.IdEncabezado')
            ->leftJoin('CatListasPrecio as c', 'c.IdListaPrecio', 'b.IdListaPrecio')
            ->leftJoin('CatTiendas as f', 'a.IdTienda', 'f.IdTienda')
            ->select(DB::raw('f.NomTienda, c.NomListaPrecio, SUM(b.CantArticulo) as kilos, SUM(b.ImporteArticulo) as importe, count(a.IdEncabezado) as clientes'))
            ->where('a.StatusVenta', 0)
            ->groupBy('f.NomTienda', 'c.NomListaPrecio')
        ...
        return view('Reportes.PorTipoDePrecio', compact('fecha'));
    }
}
