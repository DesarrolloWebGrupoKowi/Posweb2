...
class InterfazController extends Controller
{
    public function index(Request $request)
    {
        if ($request->detallado == 'on') {
            return redirect()->route('interfaz.detallado', [
            ...
            $pedidos = XXKW_HEADERS_IVENTAS::query()
                ...
                ->selectSub(function ($query) {
                    $query->from('XXKW_LINEAS_IVENTAS')
                        ->whereColumn('XXKW_LINEAS_IVENTAS.Source_Transaction_Identifier', 'XXKW_HEADERS_IVENTAS.Source_Transaction_Number')
    ...
    {
        if ($request->detallado != 'on') {
            return redirect()->route('interfaz.index', [
                'order_type' => $request->order_type,
                'fecha' => $request->fecha,
                'pedido' => $request->pedido,
                'detallado' => $request->detallado
        ...
        if ($filtrosAplicados) {
            $pedidos = XXKW_HEADERS_IVENTAS::query()
                ->with(['lineas' => function ($query) {
                    // Puedes seleccionar campos específicos de las líneas si quieres
                    $query->select('*');
                }])
                ->select(
                    'Source_Transaction_Number',
        ...
        return view('Interfaz.detallado', compact('pedidos', 'orderTypes', 'filtrosAplicados'));
    }
}
